...
envsubst '${CLIENT_MAX_BODY_SIZE} ${FILE_UPLOADS} ${MEMORY_LIMIT} ${UPLOAD_MAX_FILE_SIZE} ${POST_MAX_SIZE}' < /usr/local/etc/php/conf.d/php-upload.template > /usr/local/etc/php/conf.d/php-upload.ini
